...
\input{sections/fig-archblocks.tex}
...
Instead of shipping a neighborhood to the model, a system can have the model write a formal query. This buys exact filters, relation constraints, and joins, and it keeps the graph current without retraining. Federated bioinformatics work retrieves schemas and example queries to generate SPARQL, validates, and attempts repair on failure~\cite{emonet2024sparql}, and BioChatter exposes graph schemas to conversational models over BioCypher graphs~\cite{lobentanzer2024biochatter}.
...
KRAGEN decomposes a question through graph-of-thought prompting and retrieves per subproblem~\cite{matsumoto2024kragen}. The decomposition helps, and it commits early: if the first plan misses the decisive mechanism, later retrieval cannot recover it, which makes plan diversity and recovery from empty retrieval more informative than final accuracy. DR.KNOWS ranks candidate UMLS paths with a graph isomorphism network and iterates toward diagnosis concepts, reporting gains on MIMIC-III and an institutional dataset; its error analysis names incorrect starting concepts as a recurring cause~\cite{zuo2025drknows}, which is the clearest available demonstration that entity linking belongs inside the model rather than in preprocessing. medIKAL merges an initial model diagnosis with graph search and reranks paths~\cite{jia2025medikal}, recovering diseases absent from the retrieved paths while blurring attribution, since an answer may come from the graph, the initial guess, or their interaction. HyKGE inverts the order, generating a hypothesis first and using it to steer exploration~\cite{jiang2025hykge}, which handles underspecified questions and invites confirmation bias, because a wrong hypothesis steers retrieval toward paths that support it.
...
Across all of these, retrieval has three independent ceilings: correct seed concepts, adequate coverage, and an effective search policy. Adding hops addresses only the third and usually worsens noise and cost. A comparison that does not report coverage conditional on each ceiling, with oracle-seed and oracle-path upper bounds, cannot say which ceiling it hit.
