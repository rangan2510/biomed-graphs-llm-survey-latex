...
% ---- Revision mark-up -------------------------------------------------
% Matches the macro in main.tex. Clean by default; set REVMARKED to colour
% the additions. build.ps1 handles both builds.
\usepackage{xcolor}
\ifdefined\REVMARKED
...
\author{R. Das \and S. Ghosh \and U. B. Maulik \and S. Dasgupta \and S. Bandyopadhyay  \and U. Maulik }
\date{}
...
\rev{It also records the search and screening procedure behind the review.}
\end{abstract}

% =====================================================================
\rev{\section{Search and screening}}
\label{s:search}

\rev{Candidate work was identified through five conjunctive queries issued against four public
indexes: arXiv, PubMed, OpenAlex, and DBLP. Each query required a knowledge-graph term and a
language-model term to co-occur in a biomedical context, and PubMed and OpenAlex were filtered to
2023 onward at the query level. Searches were run on 22 August 2026;
Table~\ref{tab:ssearch} reports the yield. Deduplication resolved 823 records by DOI, 263 by arXiv
identifier, and 61 by normalised title, leaving three without a usable identifier.}

\rev{The result is a reproducible sampling frame, not a systematic review. The authors screened
titles and abstracts against the criteria in Section~II of the main paper in a single pass, without
independent duplicate assessment. We therefore report no per-stage exclusion counts. Coverage is
deliberately purposive: as explained in Section~II-C of the main paper, the review analyses a small
number of representative systems in depth rather than enumerating the frame. We make no claim of
conformance to systematic-review reporting standards such as PRISMA (Page et al., \emph{BMJ}, 2021).
Every retained reference was then checked against a canonical record. Author lists, titles, venues,
and identifiers were verified through the Crossref, arXiv, OpenAlex, DBLP, Semantic Scholar,
bioRxiv, and PubMed interfaces together with the ACL Anthology and PMLR. The publication status of
every preprint was re-checked so that peer-reviewed and preprint evidence could be distinguished at
the point of use.}

\begin{table}[!ht]
\caption{\rev{Search yield by query, run 22 August 2026. Records are counts returned by each
interface, so a work indexed in more than one place is counted more than once. The \emph{Distinct}
column reports the count after deduplication within each query. Full query strings are in the
replication log accompanying this document.}}
\label{tab:ssearch}
\centering
\small
\begin{tabularx}{\columnwidth}{@{}Yrrrrrr@{}}
\toprule
\rev{\textbf{Query}} & \rev{\textbf{arX}} & \rev{\textbf{PM}} & \rev{\textbf{OA}} & \rev{\textbf{DBLP}} & \rev{\textbf{Recs}} & \rev{\textbf{Distinct}} \\
\midrule
\rev{Q1 Knowledge graph with LLM, biomedical}   & \rev{207} & \rev{288} & \rev{366} & \rev{13} & \rev{874} & \rev{793} \\
\rev{Q2 Graph retrieval-augmented generation}   & \rev{65}  & \rev{77}  & \rev{119} & \rev{5}  & \rev{266} & \rev{245} \\
\rev{Q3 Clinical decision support and diagnosis}& \rev{88}  & \rev{40}  & \rev{126} & \rev{0}  & \rev{254} & \rev{243} \\
\rev{Q4 LLM-driven graph construction}          & \rev{118} & \rev{15}  & \rev{27}  & \rev{3}  & \rev{163} & \rev{154} \\
\rev{Q5 Graph-augmented medical question answering} & \rev{90} & \rev{40} & \rev{159} & \rev{33} & \rev{322} & \rev{298} \\
\midrule
\rev{\textbf{Across queries}} & \rev{568} & \rev{460} & \rev{797} & \rev{54} & \rev{\textbf{1879}} & \rev{\textbf{1147}} \\
\bottomrule
\end{tabularx}

\vspace{2pt}
\rev{\footnotesize Deduplication across the whole frame removed 732 of 1879 records, leaving 1147
distinct works. Of these, 105 are cited in the review; 90 of the cited works appeared in 2024 or
later, and the corpus spans 2016--2026.}
\end{table}
...
So the artifact to specify is the configuration, not the model: $C = (M, P, G, T, R, V, I)$, where
the elements denote the model and revision, the prompt and tool policy, the graph snapshot and schema,
the terminology releases, the retrieval policy, the verification layer, and the interface. Changing any element
produces a new system unless equivalence is shown.

Fig.~\ref{fig:sfailure} shows how errors propagate. A wrong edge admitted at
...
Another group needs enough information to reconstruct both the knowledge boundary and the retrieval
...
or population qualifiers drops those distinctions silently. Per-instance retrieval traces are the only way to tell a system that
...
Where licensing restrictions or patient privacy prevent release, provide hashes, synthetic examples, access
instructions, and a data dictionary. Licensed terminology strings should not be redistributed beyond
the applicable license terms, and credentialed clinical data should never appear in a public trace.
...
Binary reporting obscures important differences in reproducibility. A paper that describes an architecture and
...
R4 is the level that matters for clinical claims. Reproducing a number on the same data shows the
...
architecture. Separate the population-level graph from patient graphs, authorize access at query time
according to purpose, role, and sensitivity, and expose only the smallest subgraph that answers the
question. Treat anything derived from a patient graph as inheriting its sensitivity.
...
deployment reduces central data movement without guaranteeing privacy. Model updates, gradients,
graph statistics, and query patterns can each leak. Any privacy-enhancing technology should therefore
be evaluated against a stated threat model rather than assumed to provide privacy.

% =====================================================================
\section{Monitoring a system whose knowledge keeps changing}
\label{s:monitoring}

A single accuracy measure aggregates over too long a window and can hide stage-specific degradation.
Stronger performance at one stage may mask a decline at another. A degradation that starts in the
...
Set every threshold in advance and map a crossing to a response: investigate, roll back, suspend, or
fall back to a restricted mode. Rerun
sentinel cases after every material configuration change, since a series of individually harmless
updates can shift a system substantially without tripping any single threshold. For a dynamic evidence
...
Incident response needs traceability in both directions: from a harmful output back to the inputs,
mappings, retrieved edges, model, policy, and user actions, and from a corrupted source forward to
every release and output that depended on it. A system with only the first can explain one failure but
cannot identify every affected output.
...
graph, and link prediction cannot show that a novel hypothesis is true, still less that a drug works.
Medical question answering provides no evidence of workflow performance, patient outcomes, or freedom
from memorization. A matched text-retrieval arm is needed for retrieval benchmarks to isolate a
graph-derived benefit. Graph-comprehension benchmarks say nothing about clinical validity. Domain graph
...
arm is not. The measured benefit is then an artifact of the design, and no downstream statistical
adjustment can recover an unbiased estimate.
...
Models trained with medically harmful misinformation retained ordinary benchmark performance, so the
benchmarks missed a clinically important degradation. Removing train-test redundancies also changes
embedding-model results on biomedical graphs. A random held-out edge
