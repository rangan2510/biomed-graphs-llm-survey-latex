...
% ---- Revision mark-up -------------------------------------------------
% Matches the macro in main.tex: material added in response to review is
% wrapped in \rev{...}. Swap the definitions for an unmarked final version.
\usepackage{xcolor}
\newcommand{\rev}[1]{\textcolor{blue}{#1}}
%\newcommand{\rev}[1]{#1}   % <- swap in for the clean build
...
language-model term to co-occur, restricted to the biomedical domain, and PubMed and OpenAlex were
filtered to 2023 onward at the query level. Searches were run on 22 August 2026;
Table~\ref{tab:ssearch} reports the yield. Records were deduplicated on DOI where available, then on
arXiv identifier, then on normalised title, which resolved 823, 263, and 61 records respectively and
left three without a usable identifier.}

\rev{The result is a reproducible sampling frame, not a systematic review. Screening against the
criteria in Section~II of the main paper was performed by the authors without independent duplicate
assessment, and titles and abstracts were read in a single pass, so no per-stage exclusion counts are
reported. Coverage is deliberately purposive: the review analyses a small number of representative
systems in depth rather than enumerating the frame, for the reason given in the main paper, and we
therefore make no claim of conformance to systematic-review reporting standards such as PRISMA
(Page et al., \emph{BMJ}, 2021). Every reference retained was then checked against a canonical record.
Author lists, titles, venues, and identifiers were verified through the Crossref, arXiv, OpenAlex,
DBLP, Semantic Scholar, bioRxiv, and PubMed interfaces together with the ACL Anthology and PMLR, and
the publication status of every preprint was re-checked so that peer-reviewed and preprint evidence
could be distinguished at the point of use.}

\begin{table}[!ht]
\caption{\rev{Search yield by query, run 22 August 2026. Records are counts returned by each
interface, so a work indexed in more than one place is counted more than once; the distinct column
is after deduplication within the query. Full query strings are in the replication log accompanying
this document.}}
\label{tab:ssearch}
\centering
\small
